...
function checkforGit() {
  if (!fs.existsSync(path.dirname(".git"))) {
    process.stdout.write(
      chalk.red(
        "GIT is requried! Please install GIT and initialize in this repo"
      )
    );
    exit();
  }
...
function installStandardVersionConfig() {
  checkforGit();
  runCommand(SV_CMD);
  appendScriptInPackageJson(SET_SCRIPT_RELEASE);
}
...
function installIndividualApplication(app) {
  switch (app) {
    ...
    case HUSKY_MODULE:
      installHuskyConfig();
      return;
    case STANDARD_VERSION_MODULE:
      installStandardVersionConfig();
      return;
    default:
      exit();
  }
